...
const path = require('path'); // BRINGING THIS BACK for CSS pathing
...
// 2. Set EJS & Static Files (Using absolute path to fix nested route CSS)
app.set('view engine', 'ejs');
app.use(express.static(path.join(__dirname, 'public'))); 

// 3. Mount Headless API Routes (Stateless, so they go BEFORE sessions)
...
app.use((req, res, next) => {
    ...
    res.locals.userRole = req.session.userRole || null;    //help in identify whether user or admin
    next(); 
});
